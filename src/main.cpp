...
#if defined(NDEBUG)
# error "Recylecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfa0f04857b5103afdea83ed20f5f3c199538d70ddbde059eb75aa35dd827c1d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Recylecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Recylecoin Signed Message:\n";
...
{
    // Recylecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Recylecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Recylecoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Recylecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Recylecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Recylecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5693c152e3d5cdd47aed7029741045ca19913a2f638425c155a57c303a3e5eb5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "27/11/2022 - Odeve basladik ve Emir turgutluya dondu";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04771bba248f37e06db1a241db57c5e33840e97b354912cae40cad7ae63134db4f392cf064c69abc62d5360d55a1a40dd5341044609f39450fec95df09244c9634") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1669553942;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086187239;

        if (fTestNet)
        {
            block.nTime    = 1669553916;
            block.nNonce   = 394681849;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x35ed27bfa1bc0a3408f6e6d52bd1d94179029752946a070bdfd591a431091b7e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc7, 0xb3, 0xdc }; // Recylecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RecylecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RecylecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RecylecoinMiner(CWallet *pwallet)
{
    printf("RecylecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("recylecoin-miner");
    ...
    try { loop {
        ...
        printf("Running RecylecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RecylecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RecylecoinMiner, pwallet));
}
